...
const Requests = () => {
  ...
  const [priorityFilter, setPriorityFilter] = useState(searchParams.get("priority") || undefined);
  ...
  useEffect(() => {
    document.title = `Lab Requests - ${window.APP_NAME}`;
  }, []);
  ...
  const columns = [
    ...
    },
    {
      field: "tests_count",
      headerName: "Tests",
      valueGetter: (item) => item.tests?.length || 0,
      minWidth: { xs: 50, sm: 60 },
      renderCell: (item) => (
        <Chip
          label={`${item.tests?.length || 0} tests`}
          size="small"
          variant="outlined"
        />
      ),
  ...
  return (
    ...
    >
      <Card square variant="outlined">
        <PageHeader
          title="Lab Requests"
          subtitle="Patient laboratory requests and workflow"
          trailing={
            <Stack direction="row" spacing={1}>
              ...
              <Button
                ...
                size="small"
                startIcon={<AddIcon />}
                onClick={() => navigate("/laboratory/requests/new")}
              >
                New Request
              </Button>
            </Stack>
        ...
        <Table
          ...
          noItemsOverlayMessage="No lab requests found."
        />
      </Card>
    </Page>
  );
