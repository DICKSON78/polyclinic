...
const History = () => {
  ...
  const { data, loading, error, handleFetch } = useFetch(
    `api/laboratory/patients/${patientId}/history`,
    params,
    true,
  ...
  useEffect(() => {
    document.title = `Lab History - ${window.APP_NAME}`;
  }, []);
      ...
      renderCell: (item) => (
        <Stack direction="row" spacing={0.5} flexWrap="wrap">
          {(item.tests || []).map((t) => (
            <Chip
              key={t.id}
              label={`${t.lab_test?.name || "?"}${t.result ? `: ${t.result}` : ""}`}
              size="small"
              variant="outlined"
              color={t.is_abnormal ? "error" : "default"}
            />
          ))}
        </Stack>
      ),
        ...
        <CardContent>
          <Stack direction="row" spacing={1} alignItems="center" mb={2}>
            ...
            <Typography variant="body2" color="text.secondary">
              All laboratory requests and results for this patient.
            </Typography>
          </Stack>
          {loading && data.data.length === 0 ? (
            ...
            <Table
              ...
              noItemsOverlayMessage="No lab requests for this patient."
            />
          )}
        </CardContent>
      </Card>
